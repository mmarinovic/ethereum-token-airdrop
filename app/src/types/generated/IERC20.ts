 ...
 * IERC20 API
 */
export class IERC20 extends SoltsiceContract {
    public static get artifacts() { return require('../../../../build/contracts/IERC20.json'); }

    public static get bytecodeHash() {
        // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
        let artifacts = IERC20.artifacts;
    ...
    public static async new(deploymentParams: W3.TX.TxParams, ctorParams?: {}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<IERC20> {
        w3 = w3 || W3.default;
        if (!privateKey) {
            let contract = new IERC20(deploymentParams, ctorParams, w3, link);
            await contract._instancePromise;
            return contract;
        } else {
            let data = IERC20.newData(ctorParams, w3);
            let txHash = await w3.sendSignedTransaction(W3.zeroAddress, privateKey, data, deploymentParams);
            let txReceipt = await w3.waitTransactionReceipt(txHash);
            let rawAddress = txReceipt.contractAddress;
            let contract = await IERC20.at(rawAddress, w3);
            return contract;
        }
    }

    public static async at(address: string | object, w3?: W3): Promise<IERC20> {
        let contract = new IERC20(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    public static async deployed(w3?: W3): Promise<IERC20> {
        let contract = new IERC20('', undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }
    ...
    public static newData(ctorParams?: {}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.newDataImpl(w3, IERC20.artifacts, ctorParams ? [] : []);
        return data;
    }
